...
if leads_df.empty or "damage_type" not in leads_df.columns:
        ℹ
        return [" No lead data available to generate insights."]
# Example logic
...
def create_inspection_assignment(lead_id: str, technician_username: str, notes: str = None):
    s = get_session()
try:
    ia = InspectionAssignment(lead_id=lead_id, technician_username=technician_username,
notes=notes)
    s.add(ia)
    ...
    s.add(lead)
    s.add(LeadHistory(lead_id=lead.lead_id, changed_by="system", field="assigned_to",
old_value=str(getattr(lead, "assigned_to", "")), new_value=str(technician_username)))
    s.commit()
    return ia.id
except Exception:
    s.rollback()
...
def send_invite_email(to_email, invite_link, role):
    try:
        ...
        "to": to_email,
        "subject": "You're invited to join ReCapture Pro","html": f"""
        <h2>You're invited to ReCapture Pro</h2><p>You’ve been invited to join as <b>{role}</b>.</p>
...
try:
    ...
    if not lead:
        return False
    s.add(LeadHistory(lead_id=lead.lead_id, changed_by=actor, field="delete",
old_value=str(lead.stage), new_value="deleted"))
    s.delete(lead)
    s.commit()
    return True
except Exception:
...
try:
    ...
    {
        "lead_id": r.lead_id,
        "label": f"{r.lead_id} — {r.contact_name or 'No Name'} ({r.property_address or 'NoAddress'})"
    }
    for r in rows
    ])
finally:
...
quick = st.selectbox("Quick range", ["Today","Last 7 days","Last 30 days","90days","All","Custom"], index=4)
if quick == "Today":
...
import streamlit as st
st.markdown(
    "<div class='header'>TOTAL LEAD PIPELINE — KEY PERFORMANCEINDICATOR</div>",
    unsafe_allow_html=True
)
st.markdown(
    "<em>High-level pipeline performance at a glance. Use filters and cards to drill intodetails.</em>",
    unsafe_allow_html=True
)
# ==============================
# ALERTS
...
KPI_ITEMS = [
    ...
    ("SLA Success", f"{sla_success_pct:.1f}%", KPI_COLORS[1], "Leads contacted withinSLA"),
    ("Qualification Rate", f"{qualification_pct:.1f}%", KPI_COLORS[2], "Leads markedqualified"),
    ...
    ("Pipeline Job Value", f"${pipeline_job_value:,.0f}", KPI_COLORS[6], "Total pipeline jobvalue"),
]
r1 = st.columns(4)
r2 = st.columns(3)
cols = r1 + r2
...
r.get("created_at"), r.get("sla_hours"))
    ...
    sla_html = f"<span class='priority-time'> OVERDUE</span>" if overdue else f"<span class='small-muted'> {chr(123)}hleft{chr(125)}h left</span>"
    st.markdown(f"""
        <div class='lead-card'>
        <div style='display:flex; justify-content:space-between; align-items:center;'>
        <div>
        <div style='font-weight:800;'>#{r['lead_id']} — {r.get('contact_name') or 'Noname'}</div>
...
st.markdown("<em>Expand a lead to edit details, change status, assign owner, and createestimates.</em>", unsafe_allow_html=True)
...
index=PIPELINE_STAGES.index(lead.get("stage")) if lead.get("stage") in PIPELINE_STAGES else 0, key=f"stage_{lead['lead_id']}")
...
def get_total_leads_for_account(user):
    """
Returns total number of leads for the user's account.Adjust filtering later for multi-tenant accounts.
"""
if not user:
    return 0
with SessionLocal() as s:
...
st.markdown(
    "<em>Create or upsert a lead. All inputs are saved for reporting and CPAcalculations.</em>",
    unsafe_allow_html=True
)
# =====================================================
...
if "created_at" not in leads_df.columns:
    st.warning(" Seasonal analysis unavailable: missing created_at data. Please add newleads.")
    return
...
st.markdown("<em>This shows the Period when a lead was not contacted within the agreedService Level</em>", unsafe_allow_html=True)
today = pd.Timestamp.utcnow().date()
...
if vol_trend >= 0 and rev_trend < 0:
    signals.append(
    "Revenue is declining despite stable or increasing job volume — potential pricing orscope leakage."
    )
# Efficiency erosion
if eff_trend < -10:
    signals.append(
...
c1.markdown(f"<div class='kpi-card'><div class='kpi-title'>Total Marketing Spend</div><divclass='kpi-number' style='color:{KPI_COLORS[0]}'>${total_spend:,.2f}</div></div>",
unsafe_allow_html=True)
c2.markdown(f"<div class='kpi-card'><div class='kpi-title'>Conversions (Won)</div><divclass='kpi-number' style='color:{KPI_COLORS[1]}'>{conversions}</div></div>",
unsafe_allow_html=True)
...
c4.markdown(f"<div class='kpi-card'><div class='kpi-title'>ROI</div><div class='kpi-number'style='color:{KPI_COLORS[6]}'>${roi:,.2f} ({roi_pct:.1f}%)</div></div>",
unsafe_allow_html=True)
st.markdown("---")
...
if not agg.empty:
    fig = px.bar(agg, x="source", y=["total_spend","conversions"], barmode="group", title="TotalSpend vs Conversions by Source")
    st.plotly_chart(fig, use_container_width=True)
# ML internal page
def page_ml_internal():
    st.markdown("<div class='header'> Internal ML — Lead Scoring</div>",
unsafe_allow_html=True)
st.markdown("<em>Model runs internally and writes score back to leads. No user tuningexposed.</em>", unsafe_allow_html=True)
...
for _, r in scheduled_unassigned.iterrows():
    suggestions.append(f"Lead {r['lead_id']} is scheduled for inspection but has no assignedtechnician — assign ASAP.")
...
for _, r in high_uncontacted.iterrows():
    suggestions.append(f"High-value lead {r['lead_id']} (${int(r['estimated_value']):,}) notcontacted — prioritize contact within SLA.")
...
st.markdown(
    ...
    <div style="margin:20px auto;
        ...
        font-size:40px;
    "> </div><small>Google Review Tap Card</small>
    </div>
    """,
    unsafe_allow_html=True
)
